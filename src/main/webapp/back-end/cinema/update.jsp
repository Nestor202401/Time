...
<div class="cinema">
    ...
    <div class="seats">
        <c:forEach var="seat" items="${seats}" varStatus="status">
            <c:if test="${status.index == 0 || seat.seatRow != seats[status.index - 1].seatRow}">
                <div class="row">
            </c:if>
            ...
            <c:if test="${status.index == seats.size() - 1 || seat.seatRow != seats[status.index + 1].seatRow}">
                </div>
            </c:if>
        </c:forEach>
    </div>
...
<form id="seatForm" action="cinema.do" method="post">
    ...
    <input type="hidden" name="newStatus" value="1"> <!-- 更新后的新状态 -->
</form>
